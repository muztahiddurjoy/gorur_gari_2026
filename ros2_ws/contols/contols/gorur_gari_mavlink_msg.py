...
MAVLINK_MSG_ID_GORUR_GARI_SERIAL_MSG = 50001


class MAVLink_gorur_gari_serial_msg_message(MAVLink_message):
    """
    Gorur Gari Serial Message
    """

    id = MAVLINK_MSG_ID_GORUR_GARI_SERIAL_MSG
    msgname = "GORUR_GARI_SERIAL_MSG"
    fieldnames = ["encoder_count", "encoder_speed", "encoder_direction", "servo", "sonar_1", "sonar_2", "sonar_3", "sonar_4"]
    ordered_fieldnames = ["encoder_count", "encoder_speed", "encoder_direction", "servo", "sonar_1", "sonar_2", "sonar_3", "sonar_4"]
    ...
    def __init__(self, encoder_count: int, encoder_speed: int, encoder_direction: int, servo: int, sonar_1: int, sonar_2: int, sonar_3: int, sonar_4: int):
        MAVLink_message.__init__(self, MAVLink_gorur_gari_serial_msg_message.id, MAVLink_gorur_gari_serial_msg_message.msgname)
        self._fieldnames = MAVLink_gorur_gari_serial_msg_message.fieldnames
        self._instance_field = MAVLink_gorur_gari_serial_msg_message.instance_field
        self._instance_offset = MAVLink_gorur_gari_serial_msg_message.instance_offset
        self.encoder_count = encoder_count
        self.encoder_speed = encoder_speed
        self.encoder_direction = encoder_direction
        self.servo = servo
        self.sonar_1 = sonar_1
        self.sonar_2 = sonar_2
        self.sonar_3 = sonar_3
        self.sonar_4 = sonar_4

    def pack(self, mav: "MAVLink", force_mavlink1: bool = False) -> bytes:
        return self._pack(mav, self.crc_extra, self.unpacker.pack(self.encoder_count, self.encoder_speed, self.encoder_direction, self.servo, self.sonar_1, self.sonar_2, self.sonar_3, self.sonar_4), force_mavlink1=force_mavlink1)


# Define name on the class for backwards compatibility (it is now msgname).
# Done with setattr to hide the class variable from mypy.
setattr(MAVLink_gorur_gari_serial_msg_message, "name", mavlink_msg_deprecated_name_property())


mavlink_map: Dict[int, Type[MAVLink_message]] = {
    MAVLINK_MSG_ID_GORUR_GARI_SERIAL_MSG: MAVLink_gorur_gari_serial_msg_message,
}
...
class MAVLink(object):
    ...
    def gorur_gari_serial_msg_encode(self, encoder_count: int, encoder_speed: int, encoder_direction: int, servo: int, sonar_1: int, sonar_2: int, sonar_3: int, sonar_4: int) -> MAVLink_gorur_gari_serial_msg_message:
        """
        Gorur Gari Serial Message

        encoder_count             : count (type:uint8_t)
        encoder_speed             : speed (type:uint8_t)
        encoder_direction         : direction (type:uint8_t)
        servo                     : degree (type:uint8_t)
        sonar_1                   : cm (type:uint8_t)
        sonar_2                   : cm (type:uint8_t)
        sonar_3                   : cm (type:uint8_t)
        sonar_4                   : cm (type:uint8_t)

        """
        return MAVLink_gorur_gari_serial_msg_message(encoder_count, encoder_speed, encoder_direction, servo, sonar_1, sonar_2, sonar_3, sonar_4)

    def gorur_gari_serial_msg_send(self, encoder_count: int, encoder_speed: int, encoder_direction: int, servo: int, sonar_1: int, sonar_2: int, sonar_3: int, sonar_4: int, force_mavlink1: bool = False) -> None:
        """
        Gorur Gari Serial Message
        ...
        """
        self.send(self.gorur_gari_serial_msg_encode(encoder_count, encoder_speed, encoder_direction, servo, sonar_1, sonar_2, sonar_3, sonar_4), force_mavlink1=force_mavlink1)
